...
function workbookToCleanCsv(bytes: Uint8Array, extension: string): string {
  ...
  const prepared = sheets
    .map(sheet => {
      ...
      return csv.trim() ? `ЛИСТ: ${sheet.name}\n${csv}` : ''
    })
    .filter(Boolean)

  if(!prepared.length) throw new Error('В таблице не найдено ни одного непустого листа.')

  // Важно: расписание может быть разнесено по нескольким листам/дням.
  // Раньше выбирался только один "лучший" лист, из-за чего объединённые ячейки
  // и остальные дни терялись ещё до Gemini.
...
const REVIEW_PROMPT = `Ты — финальный контролёр школьного расписания.
Проверь JSON расписания перед сохранением.

ПРАВИЛА:
1. Не удаляй реальные уроки.
2. Если один и тот же урок в исходной таблице был общей объединённой ячейкой между несколькими классами, он должен быть у каждого из этих классов.
3. Проверь все классы 5А–11Б отдельно.
4. Не переноси уроки соседнего класса.
5. Не создавай новые уроки без основания.
6. Верни только исправленный JSON по той же схеме schedules.

Проверь:
- 10А отдельно от 10Б;
- 11А отдельно от 11Б;
- объединённые группы;
- последние строки сетки;
- кабинеты.
`
...
${csvText}`
    ...
    // Второй проход: независимая проверка результата.
    // Это ловит типичные ошибки: соседний класс, потерянная объединённая ячейка,
    // пропущенный нижний блок расписания.
    let checkedPass = firstPass
    try {
      checkedPass = await callGemini(`${REVIEW_PROMPT}

ИСПРАВЬ ЭТОТ JSON:
${JSON.stringify(firstPass)}`)
    } catch {
      // Если проверка не удалась, используем первый валидный проход.
      checkedPass = firstPass
    }

    const normalized = normalizeSchedules(checkedPass)
